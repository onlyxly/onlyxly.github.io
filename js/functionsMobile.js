...
$(function () {
    ...
    // $("#content").css("height", Math.max($loveHeart.height()+500));
    // $("#content").css("margin-top",100);
    // $("#content").css("margin-left", Math.max(($window.width() - $("#content").width()) / 2, 10));
    setInterval(function () {
        garden.render()
    }, Garden.options.growSpeed);   //控制心出来的速度
});
...
function startHeartAnimation() {
    ...
    var a = setInterval(function () {
        ...
        for (var f = 0; f < b.length; f++) {
            ...
            if (j < Garden.options.bloomRadius.max * 2) {
                e = false;
                break
            }
        }
...
function adjustWordsPosition() {
    ...
    $("#words").css("top", $("#garden").position().top + 370);
    $("#words").css("left", $("#garden").position().left + 240)
}
